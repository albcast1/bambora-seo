...
export default function ContactForm() {
  ...
  const [form, setForm] = useState({ nombre:'', telefono:'', email:'', empresa:'', mensaje:'' });
  ...
  if (status === 'ok') {
    return (
      <div className="form-success">
        <p>✓ Mensaje recibido. Te contactamos en menos de 24h.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="contact-form">
      <input name="nombre" type="text" placeholder="Nombre completo *" required value={form.nombre} onChange={handle} />
